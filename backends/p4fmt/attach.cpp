...
Visitor::profile_t Attach::init_apply(const IR::Node *node) {
...
void Attach::end_apply(const IR::Node *) {
...
bool Attach::isSystemFile(cstring file) {
    if (noIncludes) return false;
    if (file.startsWith(p4includePath)) return true;
    return false;
}

cstring Attach::ifSystemFile(const IR::Node *node) {
...
unsigned Attach::curDepth() const {
    unsigned result = 0;
    auto ctx = getContext();
    while (ctx != nullptr) {
        ctx = ctx->parent;
...
void Attach::dump(unsigned depth, const IR::Node *node, unsigned adjDepth) {
    if (!showIR) return;
...
bool Attach::preorder(const IR::P4Program *program) {
...
bool Attach::preorder(const IR::Type_Bits *t) {
    if (t->expression) {
        builder.append("bit<(");
        visit(t->expression);
        builder.append(")>");
...
bool Attach::preorder(const IR::Type_String *t) {
    builder.append(t->toString());
    return false;
}

bool Attach::preorder(const IR::Type_InfInt *t) {
    builder.append(t->toString());
    return false;
}

bool Attach::preorder(const IR::Type_Var *t) {
    builder.append(t->name);
    return false;
}

bool Attach::preorder(const IR::Type_Unknown *) {
    BUG("Cannot emit code for an unknown type");
    // builder.append("*unknown type*");
    return false;
}

bool Attach::preorder(const IR::Type_Dontcare *) {
    builder.append("_");
    return false;
}

bool Attach::preorder(const IR::Type_Void *) {
    builder.append("void");
    return false;
}

bool Attach::preorder(const IR::Type_Name *t) {
    visit(t->path);
    return false;
}

bool Attach::preorder(const IR::Type_Stack *t) {
    dump(2);
    visit(t->elementType);
    builder.append("[");
    visit(t->size);
...
bool Attach::preorder(const IR::Type_Specialized *t) {
    dump(3);
    visit(t->baseType);
    builder.append("<");
    setVecSep(", ");
...
bool Attach::preorder(const IR::Argument *arg) {
    dump(2);
...
bool Attach::preorder(const IR::Type_Typedef *t) {
    dump(2);
...
bool Attach::preorder(const IR::Type_Newtype *t) {
    dump(2);
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::Type_BaseList *t) {
    dump(3);
...
bool Attach::preorder(const IR::P4ValueSet *t) {
    dump(1);
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::Type_Enum *t) {
    dump(1);
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::Type_SerEnum *t) {
    dump(1);
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::TypeParameters *t) {
    if (!t->empty()) {
        builder.append("<");
        bool first = true;
...
bool Attach::preorder(const IR::Method *m) {
    dump(1);
...
bool Attach::preorder(const IR::Function *function) {
    dump(1);
...
bool Attach::preorder(const IR::Type_Extern *t) {
    dump(2);
    if (isDeclaration) {
        if (!t->annotations->annotations.empty()) {
            visit(t->annotations);
...
bool Attach::preorder(const IR::Type_Boolean *) {
    builder.append("bool");
    return false;
}

bool Attach::preorder(const IR::Type_Varbits *t) {
    if (t->expression) {
...
bool Attach::preorder(const IR::Type_Package *package) {
    dump(2);
    builder.emitIndent();
...
bool Attach::process(const IR::Type_StructLike *t, const char *name) {
    dump(2);
    if (isDeclaration) {
        builder.emitIndent();
        if (!t->annotations->annotations.empty()) {
...
bool Attach::preorder(const IR::Type_Parser *t) {
    dump(2);
    builder.emitIndent();
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
...
bool Attach::preorder(const IR::Type_Control *t) {
    dump(2);
    builder.emitIndent();
    if (!t->annotations->annotations.empty()) {
        visit(t->annotations);
...
bool Attach::preorder(const IR::Constant *c) {
...
bool Attach::preorder(const IR::BoolLiteral *b) {
    builder.append(b->toString());
    return false;
}

bool Attach::preorder(const IR::StringLiteral *s) {
    builder.append(s->toString());
    return false;
}

bool Attach::preorder(const IR::Declaration_Constant *cst) {
    dump(2);
...
bool Attach::preorder(const IR::Declaration_Instance *i) {
    dump(3);
...
bool Attach::preorder(const IR::Declaration_Variable *v) {
    dump(2);
...
bool Attach::preorder(const IR::Type_Error *d) {
    dump(1);
    bool first = true;
...
bool Attach::preorder(const IR::Declaration_MatchKind *d) {
    dump(1);
...
#define VECTOR_VISIT(V, T)                                    \
    bool Attach::preorder(const IR::V<IR::T> *v) {            \
...
bool Attach::preorder(const IR::Slice *slice) {
...
bool Attach::preorder(const IR::DefaultExpression *) {
...
bool Attach::preorder(const IR::This *) {
    builder.append("this");
    return false;
}

bool Attach::preorder(const IR::PathExpression *p) {
    visit(p->path);
    return false;
}

bool Attach::preorder(const IR::TypeNameExpression *e) {
    visit(e->typeName);
    return false;
}

bool Attach::preorder(const IR::ConstructorCallExpression *e) {
...
bool Attach::preorder(const IR::Member *e) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::SelectCase *e) {
    dump(2);
    int prec = expressionPrecedence;
    expressionPrecedence = DBPrint::Prec_Low;
    setListTerm("(", ")");
...
bool Attach::preorder(const IR::SelectExpression *e) {
...
bool Attach::preorder(const IR::ListExpression *e) {
    cstring start, end;
...
bool Attach::preorder(const IR::P4ListExpression *e) {
...
bool Attach::preorder(const IR::NamedExpression *e) {
...
bool Attach::preorder(const IR::StructExpression *e) {
    if (expressionPrecedence > DBPrint::Prec_Prefix) builder.append("(");
...
bool Attach::preorder(const IR::HeaderStackExpression *e) {
    if (expressionPrecedence > DBPrint::Prec_Prefix) builder.append("(");
...
bool Attach::preorder(const IR::Invalid *) {
    builder.append("{#}");
    return false;
}

bool Attach::preorder(const IR::Dots *) {
    builder.append("...");
    return false;
}

bool Attach::preorder(const IR::NamedDots *) {
    builder.append("...");
    return false;
}

bool Attach::preorder(const IR::InvalidHeader *e) {
    if (expressionPrecedence > DBPrint::Prec_Prefix) builder.append("(");
    builder.append("(");
    visit(e->headerType);
    builder.append(")");
...
bool Attach::preorder(const IR::InvalidHeaderUnion *e) {
    if (expressionPrecedence > DBPrint::Prec_Prefix) builder.append("(");
    builder.append("(");
...
bool Attach::preorder(const IR::MethodCallExpression *e) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::Operation_Binary *b) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::Mux *b) {
    int prec = expressionPrecedence;
    bool useParens = prec >= b->getPrecedence();
    if (useParens) builder.append("(");
    expressionPrecedence = b->getPrecedence();
...
bool Attach::preorder(const IR::Operation_Unary *u) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::ArrayIndex *a) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::Cast *c) {
    int prec = expressionPrecedence;
...
bool Attach::preorder(const IR::AssignmentStatement *a) {
    dump(2);
    visit(a->left);
    builder.append(" = ");
    visit(a->right);
...
bool Attach::preorder(const IR::BlockStatement *s) {
    dump(1);
...
bool Attach::preorder(const IR::BreakStatement *) {
    dump(1);
...
bool Attach::preorder(const IR::ContinueStatement *) {
    dump(1);
...
bool Attach::preorder(const IR::ExitStatement *) {
    dump(1);
    builder.append("exit");
    builder.endOfStatement();
    return false;
...
bool Attach::preorder(const IR::ReturnStatement *statement) {
    dump(2);
...
bool Attach::preorder(const IR::EmptyStatement *) {
    dump(1);
    builder.endOfStatement();
    return false;
}

bool Attach::preorder(const IR::IfStatement *s) {
    dump(2);
    builder.append("if (");
    visit(s->condition);
    builder.append(") ");
...
bool Attach::preorder(const IR::ForStatement *s) {
    dump(2);
    if (!s->annotations->annotations.empty()) {
        visit(s->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::ForInStatement *s) {
    dump(2);
    if (!s->annotations->annotations.empty()) {
        visit(s->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::MethodCallStatement *s) {
    dump(3);
    visit(s->methodCall);
    builder.endOfStatement();
    return false;
...
bool Attach::preorder(const IR::SwitchCase *s) {
    visit(s->label);
    builder.append(": ");
    visit(s->statement);
    return false;
...
bool Attach::preorder(const IR::SwitchStatement *s) {
    dump(4);
...
bool Attach::preorder(const IR::Annotations *a) {
    bool first = true;
...
bool Attach::preorder(const IR::Annotation *a) {
    builder.append("@");
...
bool Attach::preorder(const IR::Parameter *p) {
    dump(2);
...
bool Attach::preorder(const IR::P4Control *c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool Attach::preorder(const IR::ParameterList *p) {
    builder.append("(");
    bool first = true;
...
bool Attach::preorder(const IR::P4Action *c) {
    dump(2);
...
bool Attach::preorder(const IR::ParserState *s) {
    dump(1);
...
bool Attach::preorder(const IR::P4Parser *c) {
    dump(1);
    bool decl = isDeclaration;
    isDeclaration = false;
    visit(c->type);
...
bool Attach::preorder(const IR::ExpressionValue *v) {
    dump(2);
    visit(v->expression);
    builder.endOfStatement();
    return false;
...
bool Attach::preorder(const IR::ActionListElement *ale) {
    dump(3);
...
bool Attach::preorder(const IR::ActionList *v) {
    dump(2);
    builder.blockStart();
    setVecSep(";\n", ";\n");
...
bool Attach::preorder(const IR::Key *v) {
    dump(2);
    builder.blockStart();
...
bool Attach::preorder(const IR::Property *p) {
    dump(1);
    if (!p->annotations->annotations.empty()) {
        visit(p->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::TableProperties *t) {
...
bool Attach::preorder(const IR::EntriesList *l) {
    dump(1);
    builder.append("{");
    builder.newline();
    builder.increaseIndent();
...
bool Attach::preorder(const IR::Entry *e) {
    dump(2);
    builder.emitIndent();
...
bool Attach::preorder(const IR::P4Table *c) {
    dump(2);
    if (!c->annotations->annotations.empty()) {
        visit(c->annotations);
        builder.spc();
...
bool Attach::preorder(const IR::Path *p) {
    builder.append(p->asString());
    return false;
}

std::string attach(const IR::INode *node) {
    std::stringstream stream;
    P4::Attach attach(&stream, false);
    node->getNode()->apply(attach);
    return stream.str();
}

void dumpNodeP4(const IR::INode *node) {
    auto s = attach(node);
    std::cout << s;
}
